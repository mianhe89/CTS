/*
변수 선언하기(declaration)
변수(Variable)는 이름이 붙은 저장소를 의미합니다. 자바스크립트 (그리고 다른 컴퓨터 언어에서도) 변수는 컴퓨터 메모리에 자리를 잡게 되고,
여러 값을 넣을 수 있습니다. 이렇게 하나의 변수가 자리 잡는 동작을 "변수를 선언(Declaration)한다." 라고 합니다.
...
선언은 이제 여기는 내 땅이야! 라고 "선언"하는 동작입니다. 즉, word라는 저장소를 만드는 작업을 배운겁니다. 이번에는 course라는 변수를 선언해봅시다!
 오른쪽 창의 주석(Comment)를 확인하고, 그에 맞추어 우측 하단에서 테스트 실행과 제출을 해주세요.
*/
